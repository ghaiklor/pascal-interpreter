...
describe('Interpreter', () => {
  it('Should properly handle creating new instance', () => {
    const interpreter = new Interpreter('BEGIN END.');

    assert.instanceOf(interpreter.parser, Parser);
    assert.instanceOf(interpreter.ast, AST.Compound);
  });

  it('Should properly get a visitor for specified node', () => {
    const number = AST.Number.create(Token.create(Token.INTEGER, 500));
    const interpreter = new Interpreter('BEGIN END.');
